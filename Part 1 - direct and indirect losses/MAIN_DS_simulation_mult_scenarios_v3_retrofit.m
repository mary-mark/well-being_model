...
%     HH_agg_bldg_num = zeros(nlocs,n_agg_occ);
    ...
    for scen = scenario_flag%:(length(SCENARIOS))
        output_file = sprintf('Output/Damage states/DS_results_%s_sc%i_%i_DEC2018_retrofit.mat',fault,SCENARIOS_descrip(scen,1),SCENARIOS_descrip(scen,2));
        ...
        for i = 1:nlocs%:nlocs_part;
            ...
            for j = 1:nbldg_type
                ...
                if ismember(BayArea.bldg_occType{j},{'RES3'})
                    idx_designLevel = 1;
                end
