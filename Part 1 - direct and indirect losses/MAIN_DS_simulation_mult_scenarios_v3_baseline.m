...
fault = 'Hayward_extra';
scanario_flags = [1,5,6];
for scenario_flag= scanario_flags 
    scenario_flag% = 4; %1,5,6
...
%     HH_agg_bldg_num = zeros(nlocs,n_agg_occ);
    ...
    for scen = scenario_flag%:(length(SCENARIOS))
        output_file = sprintf('Output/Damage states/DS_results_%s_sc%i_%i_DEC2018_baseline.mat',fault,SCENARIOS_descrip(scen,1),SCENARIOS_descrip(scen,2));
