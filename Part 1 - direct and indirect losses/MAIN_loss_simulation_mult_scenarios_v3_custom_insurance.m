...
load './Input data/Exposure/insurance_penetration.mat'
...
output_suffix = '_ins_rich_50';
[~,tract_index] = ismember(BayArea.CensusTract,ins_penetr_data.CensusTracts); 
correct_tract = ins_penetr_data.CensusTracts(tract_index);

switch output_suffix
    case '_ins_poor_15'
penetr_vector = ins_penetr_data.poor_15(tract_index);
    case '_ins_poor_30'
        penetr_vector = ins_penetr_data.poor_30(tract_index);
     case '_ins_poor_50'
        penetr_vector = ins_penetr_data.poor_50(tract_index);
    case '_ins_rich_15'
        penetr_vector = ins_penetr_data.rich_15(tract_index);
    case '_ins_rich_30'
        penetr_vector = ins_penetr_data.rich_30(tract_index);
    case '_ins_rich_50'
        penetr_vector = ins_penetr_data.rich_50(tract_index);
    otherwise
        error('NOT THE RIGHT SUFFIX')
end
  
if sum(correct_tract == BayArea.CensusTract) ~= length(correct_tract)
   error('tract_mapping didnt work')
end
    ...
    for i = 1:length(agg_occ)
        ...
        for j = 1:nlocs
            for k = 1:nsims
                ...
                total_loss = 0;
                ins_loss_perDS = 0;
                ins_penetration = penetr_vector(j);
