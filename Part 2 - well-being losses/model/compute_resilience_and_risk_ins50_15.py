...
def launch_compute_resilience_and_risk_thread(myCountry,pol_str='',optionPDS='no', 
                                              labourIncome = False,output_folder = '',
                                              suffix = '',nsims = None):
    ...
    # Setup directories
    output       = model+'/../output_country/'+myCountry+'_'+str(nsims)+'sims/'+output_folder
    intermediate = model+'/../intermediate/'+myCountry+suffix+'_'+str(nsims)+'sims/'
    if not os.path.exists(output):
        os.makedirs(output)
    if not os.path.exists('optimization_libs/'+myCountry+pol_str+'_'+str(nsims)+"sims/"):
        os.makedirs('optimization_libs/'+myCountry+pol_str+'_'+str(nsims)+"sims/")
    ...
    income_cats   = 'tract'           #for Bay Area: 'tract', for the rest 'hhid'  #categories of households
    
    affected_cats = pd.Index(['a', 'na'], name='affected_cat') 
    affected_cats = pd.Index(['a'], name='affected_cat') #pd.Index(['a', 'na'], name='affected_cat')           #categories for social protection
    helped_cats   = pd.Index(['helped','not_helped'], name='helped_cat')
    helped_cats   = pd.Index(['not_helped'], name='helped_cat')
    ...
    macro_event, cats_event, hazard_ratios_event = compute_with_hazard_ratios(myCountry,pol_str,intermediate+'hazard_ratios.csv',
                                                                              macro,cat_info,economy,event_level,income_cats,
                                                                              default_rp,rm_overlap=False,verbose_replace=True,suffix = suffix)
    gc.collect()
    print('A')
    #verbose_replace=True by default, replace common columns in macro_event and cats_event with those in hazard_ratios_event
    
    # If considering labour income prepare the labour recovery dataframe
    x_max = 10 # 10 years
    nsims = len(cats_event.index.levels[2])
    fault_identifier = 'Hayward_sc28_6'
    labour_event = get_labour_df(cats_event,intermediate,nsims,x_max,fault_identifier)
    ...
    macro_event, cats_event_ia, pub_costs_inf = compute_dK(pol_str,macro_event,cats_event,event_level,affected_cats,myCountry,optionPDS,share_public_assets,
                                                           labourIncome,labour_event,suffix = suffix, nsims = nsims,output_folder = output_folder) 
    ...
    macro_event, cats_event_iah, pub_costs_pds = calculate_response(myCountry,pol_str,macro_event,cats_event_ia,pub_costs_pds,event_level,helped_cats,default_rp,
                                                                    ...
                                                                    loss_measure='dk_private',fraction_inside=1, share_insured=.25,labourIncome = labourIncome, 
                                                                    labour_event = labour_event, suffix = suffix, nsims = nsims,output_folder =output_folder)
    print('C\n\n')
    ...
    out = calc_dw_inside_affected_province(myCountry,pol_str,optionPDS,macro_event,cats_event_iah,event_level,
                                           option_CB,return_stats=False,return_iah=True,is_revised_dw=is_rev_dw,
                                           labourIncome = labourIncome, labour_event = labour_event, nsims = nsims,
                                           suffix = suffix,output_folder=output_folder)
    print('F')

    # Flag: running local welfare
    print('running national welfare')
    results,iah = process_output(myCountry,pol_str,out,macro_event,economy,default_rp,True,is_local_welfare,is_rev_dw)
    print('G')
...
if __name__ == '__main__':
    ...
    if myCountry == 'BA':
        ...
        output_folder = 'policy - insurance'
        pds_str = 'no'
        debug = True
        labourIncome = True
        pol_str ='_ins50_15'# ('_baseline',no_code','retrofit','no_UI','UI_2yrs','no_ins','ins50_15',_ins_rich_50)
        suffix = '_ins50_15'# ('_baseline',no_code','retrofit','no_ins','ins50_15',_ins_rich_50)
        nsims = 500
        
        if debug:# debug:
            if labourIncome:
                 print('Running in debug mode! Including labour income')
            else: 
                 print('Running in debug mode! NOT including labour income')
            launch_compute_resilience_and_risk_thread(myCountry,pol_str,pds_str,labourIncome, output_folder,suffix = suffix,nsims = nsims)
            
        else:
            with Pool() as pool: # this is for parallel
                print('LAUNCHING',len(list(product([myCountry],pol_str,pds_str,labourIncome, output_folder))),'THREADS:\n',list(product([myCountry],pol_str,pds_str,labourIncome, output_folder)))
                pool.starmap(launch_compute_resilience_and_risk_thread, list(product([myCountry],pol_str,pds_str,labourIncome, output_folder)))
