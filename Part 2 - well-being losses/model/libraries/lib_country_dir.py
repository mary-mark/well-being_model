...
# Define directories
def set_directories(myCountry,suffix = '',nsims = -999):  # get current directory
    global inputs, intermediate
    inputs        = model+'/../inputs/'+myCountry+'/'       # get inputs data directory
    if nsims==-999:
        intermediate  = model+'/../intermediate/'+myCountry+suffix+'/'
    else:
        intermediate  = model+'/../intermediate/'+myCountry+suffix+'_'+str(nsims)+'sims/' # get outputs data directory
...
def get_places(myC,economy):
    ...
    if myC == 'MW':
        ...
        return df
    if myC == 'BA':
        df = pd.read_csv(inputs+'/Bay_area_general.csv').set_index('county_id').dropna(how='all')['population'].sum(level='county_id').to_frame()
        df.columns = ['population']
        return df
    else: 
        return None
...
def get_places_dict(myC):
    ...
    if myC == 'FJ':
        ...
        raw_data = pd.read_csv(inputs+'/Bay_area_general.csv',usecols = ['county_id','county_name',]).set_index('county_id')
        p_code = raw_data.groupby('county_id').first()
...
def get_subsistence_line(myC,df_pov = None):
    ...
    if myC == 'PH':
		...
		raise('Something went wrong since poverty level should be defined in cat_info')
		return 0
...
def get_avg_prod(myC):
    ...
    elif myC == 'BA': return 0.318736853
